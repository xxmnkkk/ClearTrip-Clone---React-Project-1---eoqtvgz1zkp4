...
function Header() {
    // This is the code for my header section
    // I am importing all of the state that i want to use in this component below
    const { isLoggedIn, setIsLoggedIn, loginModal, setLoginModal, profileModal, setProfileModal } = useContext(AuthContext);

    // just updating the loggedin state so that whatever is required to be shown gets updated accordingly
    useEffect(() => {
    ...
    // Just defininng some functions for acting upon click of some html elements
    ...
    {/* Some basic code for the ui of my header layout */}
        ...
        {loginModal && <LoginModal onClose={() => setLoginModal(false)} onLoginSuccess={handleLoginSuccess} />}
        {profileModal && <Profile onClose={closeProfileModal} onLogout={handleLogout} />}
    </>
